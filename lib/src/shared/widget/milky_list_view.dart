...
class MilkyListView<T> extends StatefulWidget {
  const MilkyListView({
    ...
    super.key,
          ...
          onLoadMoreData == null && controller == null,
          'controller can not be null when you use onLoadMoreData',
        );
...
class _MilkyListViewState<T> extends State<MilkyListView<T>> {
  ...
  void _addControllerListener() => widget.controller?.addListener(
        _listener,
      );

  void _removeControllerListener() => widget.controller?.removeListener(
        _listener,
      );

  Future<void> _listener() async {
    if (widget.hasMoreData && _reachedToBottom && showLoadMoreData == false) {
      setState(() {
